...
"""
version 3
completely remove DataArea._area, scaling of scene and cosmetic pens
 - we need to redraw every time we resize Data Area and every time we zoom :(
...
class GS(object):
    def __init__(self):
        ...
        self.dataLineWidth = 1.5 #WARNING: this will not scale with the rest, you should put the width that will look good in the final resolution
...
class MovableRelativeAnchorRY(Anchor):
    ...
    position within parent item, which will be updated when parent chages size
    ...
    def setPos(self, x, y):
        print("RelativeAnchorRY.setPos", x, y, self.pos())
        self._x = x*1j
        self._y = (1-y)*1j
        self._updatePos()
        pass
    ...
    def setX(self, x):
        print("RelativeAnchorRY.setX", x, self.pos())
        raise UserError()
        self._x = x*1j
#        self._y = (1-self.y())*1j #theses are not updated automatically
    ...
    def setY(self, y):
        print("RelativeAnchorRY.setY", y, self.pos())
#        self._x = self.x()*1j #theses are not updated automatically
    ...
    def pos(self):
        return QtCore.QPointF(self._x.imag, self._y.imag)
...
#        return 1-super().y()/h if h>0 else 0
    ...
    def _updateFromPos(self, *args):
        ...
        y = 1-pos.y()/h if h>0 else 0
        self._x = x*1j
        self._y = y*1j
        self.posChanged.emit(super().pos())
        print("MovableRelativeAnchorRY._updateFromPos", self, self._x, self._y, self.pos())
...
class AnchorRectItem(QtWidgets.QGraphicsRectItem):
    ...
    def _update(self, *args):
        s = self._tl.pos()
        e = self._br.pos()
#        print("AnchorRectItem._update", s, e, e.x()-s.x(), e.y()-s.y())
...
class PlotArea(Params, QtWidgets.QGraphicsWidget):
    ...
    def __init__(self, *args, backgroundColor=None, name=None, **kw):
        ...
        print("\tPlotArea.bottomLeft.parentItem()", self.bottomLeft.parentItem())
...
class DataArea(PlotArea):
    ...
    def wheelEvent(self, ev):
        ...
        print("DataArea.wheelEvent", ev.pos(), ev.scenePos(), br, ev.pos().x()/br.width(), ev.pos().y()/br.height())
        p = self.parentItem()
        if p is not None:
...
class AxisArea(PlotArea):
    ...
    def __init__(self, transform=None, position=AxisPosition.bottom, units=None, label=None, tickFormat="{:.1f}", tickNum=7, *args, **kw):
        ...
        for t in self._ticksArea:
            # ~ tanchor = self.start+t*(self.end-self.start) #this will create intermediate anchors which will not be destroyed
            tanchor = (1.-t)*self.start.anchorPos()+t*self.end.anchorPos() 
            tanchor = Anchor(*tanchor, self, color="grey")
            # ~ print("\t tanchor",tanchor.anchorPos(), tanchor.pos()) 
            tend = GS.tickLength*self.normal
            tline = QtWidgets.QGraphicsLineItem(QtCore.QLineF(QtCore.QPointF(0,0), tend), tanchor)
            tline.setPen(GS.plotLinePen)
            self._tickAnchors.append(tanchor)
            ...
            tlabel.setFont(f)
            # ~ tend += GS.tickMinorSpacing*self.normal
            tlabel.setPos((tend if GS.tickLength>0 else QtCore.QPointF(0,0))+GS.tickMinorSpacing*self.normal)
    ...
    def _updateTicks(self, *args):
        ...
        for t, tlabel in zip(t, self._tickLabels):
            ...
            #left:  -1, -0.5
            #right: 0, -0.5
            dx, dy = self._tickLabelShift
            tlabel.setTransform(QtGui.QTransform.fromTranslate(br.width()*dx, br.height()*dy), False)
    ...
    def _preferredHeight(self):
        ...
        h += max(GS.tickLength, 0)
        
        if len(self._tickLabels) > 0:
            #if we have tick labels, highest tick label
            h += max([it.boundingRect().height() for it in self._tickLabels])+GS.tickMinorSpacing
    ...
    def _preferredWidth(self):
        ...
        # but it will also mess up tick label position
        #TODO: nevertheless it could be usefull and could be handled
        w += max(GS.tickLength, 0)
        
        if len(self._tickLabels) > 0:
            #if we have tick labels, highest tick label
            w += max([it.boundingRect().width() for it in self._tickLabels])+GS.tickMinorSpacing
    ...
    def mouseMoveEvent(self, ev):
        ...
        print("\t mouse move", ev.pos().x(), ev.pos().y(), ev.lastPos().x()/self.boundingRect().width())        
        if ev.buttons() == QtCore.Qt.LeftButton:
            if self.position.isHorizontal():
                lpx = ev.lastPos().x()
    ...
    def _setZoomRange(self, lmn, lmx):
        ...
        if lmx is not None and lmx!=self._zoomLmx: 
            ...
            emit = True
        #sanitize this
        if self._zoomLmn>self._zoomLmx: self._zoomLmn, self._zoomLmx = self._zoomLmx, self._zoomLmn
    ...
    def zoomToData(self, data=None, verbose=False):
        ...
        print("AxisArea.zoomToData", self._name, self._dataRange, None if data is None else self._dataRanges[data])
        if data is None:
...
class ColorAxisArea(AxisArea):
    ...
    def __init__(self, *args, **kw):
        ...
        # ~ print("self._colorBarWidth", self._colorBarWidth)
        # ~ print("labelAnchor", self.labelAnchor.anchorPos(), self.labelAnchor.pos())
        # ~ for a in [self.start, self.end, self.labelAnchor]+self._tickAnchors:
        for a in [self.start, self.end]+self._tickAnchors:
            if self.position == AxisPosition.right:
...
class SmoothPathItem(QtWidgets.QGraphicsPathItem):
    ...
    def shape(self):
        ...
        print("\t\tSmoothPathItem.shape", self.parentItem().parentItem().scale())
...
class SeriesPlotter(XYPlotter): 
    ...
    def _setPenWidth(self, width):
        print("SeriesPlotter._setPenWidth", width, width*GS.dataLineWidth)
    ...
    def _setData(self, x, y=None):
        ...
        print("setting pen with width", self._pen.widthF())
...
#        print("OriginDecorator.contains", args, ttt, self.isClipped())
#        return ttt
#
    def shape(self, *args):
#        ttt = self.marker.shape()
#        print("OriginDecorator.shape", ttt.boundingRect())
#        return ttt
        return self.marker.shape()

#    def collidesWithPath(self, *args):
#        print("OriginDecorator.collidesWithPath", self, args)
...
#        res = self.childrenBoundingRect()
        ...
        #... it likely does not consider cosmetic pens
        # which is apparent old issue with Qt and not likely to be resolved any time soon :(
#        print("Decorator.boundingRect", res, self)
        return self.childrenBoundingRect()
    ...
    def _setOrigin(self, origin):
        ...
        print("OriginDecorator._setOrigin", origin, self.pos())
        pass
    ...
    def _updateXL(self):
        ...
        old = self._xL
        print("OriginDecorator._updateXL", self.x(), self._xaxis.area2linear(self.x()), old)
    ...
    def _updateXA(self, *args):
        ...
        print("OriginDecorator._updateXA", self._xL, self._xaxis.linear2area(self._xL))
...
class Plot(QtWidgets.QGraphicsWidget):
    ...
    def resizeEvent(self, *args):
        # ~ print("\tPlot.resizeEvent", args, self.geometry(), self.parentItem().geometry())
        # ~ pol = self._da.sizePolicy()
        # ~ print("\t\tda", self._da.geometry(), self._da.preferredSize(), self._da.minimumSize(), self._da.maximumSize(), pol.horizontalPolicy(), pol.verticalPolicy(), pol.horizontalStretch(), pol.verticalStretch())
        # ~ try:
            # ~ print("\t\tx1", self.x1.geometry())
            # ~ pol = self.x1.sizePolicy()
            # ~ print("\t\tda", self.x1.geometry(), self.x1.preferredSize(), self.x1.minimumSize(), self.x1.maximumSize(), pol.horizontalPolicy(), pol.verticalPolicy(), pol.horizontalStretch(), pol.verticalStretch())
        # ~ except:
            # ~ pass
        # ~ try:
            # ~ print("\t\ty1", self.y1.geometry())
        # ~ except:
            # ~ pass
        # ~ try:
            # ~ print("\t\tx2", self.x2.geometry())
        # ~ except:
            # ~ pass
        # ~ try:
            # ~ print("\t\ty2", self.y2.geometry())
        # ~ except:
            # ~ pass
    ...
    def _panDataArea(self, lastPos, pos):
        for ap in self._axes:
            if self._axes[ap]:
                ...
                # ~ print("Plot._panDataArea", ap.position.isHorizontal(), d)
                ap.pan(d)
        pass
...
#        print()
    ...
    def saveImage(self, filepath=None, width=None, height=None, *args, **kw):
        ...
        print("FigureScene.saveImage:", filepath, width, height)
        try:
...
#        plot1.addDecorator(RectDecorator(plot1, color="purple"))
